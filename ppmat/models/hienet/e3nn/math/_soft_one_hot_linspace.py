...
def soft_one_hot_linspace(
    ...
    .. jupyter-execute::
        ...
        import torch
        from ppmat.models.hienet.e3nn.math import soft_one_hot_linspace
    ...
    if not cutoff:
        values = paddle.linspace(start, end, number, dtype=x.dtype, device=x.device)
        step = values[1] - values[0]
    else:
        values = paddle.linspace(start, end, number + 2, dtype=x.dtype, device=x.device)
        step = values[1] - values[0]
        values = values[1:-1]
    ...
    if basis == "cosine":
        return paddle.cos(math.pi / 2 * diff) * (diff < 1) * (-1 < diff)
    if basis == "smooth_finite":
        return (
            1.14136
            * paddle.exp(paddle.tensor(2.0))
    ...
    if basis == "fourier":
        ...
        if not cutoff:
            i = paddle.arange(0, number, dtype=x.dtype, device=x.device)
            return paddle.cos(math.pi * i * x) / math.sqrt(0.25 + number / 2)
        else:
            i = paddle.arange(1, number + 1, dtype=x.dtype, device=x.device)
            return (
                paddle.sin(math.pi * i * x)
                / math.sqrt(0.25 + number / 2)
                * (0 < x)
                * (x < 1)
            )
    if basis == "bessel":
        ...
        c = end - start
        bessel_roots = (
            paddle.arange(1, number + 1, dtype=x.dtype, device=x.device) * math.pi
        )
        out = math.sqrt(2 / c) * paddle.sin(bessel_roots * x / c) / x
        if not cutoff:
            return out
        else:
