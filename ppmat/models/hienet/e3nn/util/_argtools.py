...
def _transform(
    ...
    translation = paddle.as_tensor(translation, dtype=transform_dtype)
    for irreps, a in zip(irreps_dat, dat):
        ...
        if irreps is None:
            ...
            translation = paddle.as_tensor(translation, device=a.device)
            out.append(
                (a.to(transform_dtype) @ rot_mat.T.to(a.device) + translation).to(
                    out_dtype
                )
            )
        else:
...
def _get_io_irreps(func, irreps_in=None, irreps_out=None):
    ...
    if (irreps_in is None or irreps_out is None) and isinstance(
        func, type(None),  # torch.jit.ScriptModule -> type(None) (always False)
    ):
        warnings.warn(
...
def _get_device(mod: paddle.nn.Module) -> paddle.device:
    # parameters()/buffers() return lists (not iterators)
    params = list(mod.parameters())
    a_buf = params[0] if params else None
    if a_buf is None:
        bufs = list(mod.buffers())
        a_buf = bufs[0] if bufs else None
    return a_buf.place if a_buf is not None else "cpu"


def _get_floating_dtype(mod: paddle.nn.Module) -> paddle.dtype:
    ...
    if a_buf is None:
        for buf in mod.buffers():
            if buf.is_floating_point():
                a_buf = buf
                break
    return a_buf.dtype if a_buf is not None else paddle.get_default_dtype()
