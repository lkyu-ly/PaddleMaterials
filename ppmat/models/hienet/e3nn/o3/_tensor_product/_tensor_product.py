...
from ._codegen import (EagerNotCompiledLeftRight, EagerRightPassthrough,
...
@compile_mode("script")
class TensorProduct(CodeGenMixin, paddle.nn.Module):
    ...
    def __init__(
        ...
        if compile_left_right:
            ...
            # Not compiled: the stub raises the runtime error on call.
            graphmod_left_right = EagerNotCompiledLeftRight()
        if compile_right:
            ...
            # Not compiled: passthrough stub (forward(x2, w) -> x2).
            graphmod_right = EagerRightPassthrough()
        ...
        if internal_weights and self.weight_numel > 0:
            ...
            self.weight = paddle.nn.Parameter(paddle.randn(self.weight_numel))
        else:
    ...
    def _prep_weights_python(
        ...
        if isinstance(weight, list):
            ...
            return paddle.cat(weight, dim=-1)
        else:
            return weight
    ...
    def right(self, y, weight: Optional[paddle.Tensor] = None):
        """Partially evaluate :math:`w x \\otimes y`.
