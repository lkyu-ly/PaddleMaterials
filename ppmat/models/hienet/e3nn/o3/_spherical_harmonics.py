...
@compile_mode("script")
class SphericalHarmonics(paddle.nn.Module):
    ...
    def __init__(
        ...
        if get_optimization_defaults()["jit_mode"] == "script":
            self.sph_func = paddle.jit.to_static(function=_spherical_harmonics)
        elif get_optimization_defaults()["jit_mode"] == "inductor":
            self.sph_func = _spherical_harmonics  # inductor mode: eager function
        else:
            self.sph_func = _spherical_harmonics

    def forward(self, x: paddle.Tensor) -> paddle.Tensor:
        if self.normalize:
            x = x / paddle.sqrt((x * x).sum(-1, keepdim=True))
        ...
        if self.normalization == "integral":
            # scalar division via the out-of-place form
            ...
            sh.div_(
                paddle.cat(
                    [
                        (
                            math.sqrt(2 * l + 1)
                            * paddle.ones(2 * l + 1, dtype=sh.dtype, device=sh.device)
                        )
...
def _spherical_harmonics(
    ...
    sh_0_0 = paddle.ones_like(x)
    if lmax == 0:
        return paddle.stack([sh_0_0], dim=-1)
    sh_1_0 = math.sqrt(3) * x
    sh_1_1 = math.sqrt(3) * y
    sh_1_2 = math.sqrt(3) * z
    if lmax == 1:
        return paddle.stack([sh_0_0, sh_1_0, sh_1_1, sh_1_2], dim=-1)
