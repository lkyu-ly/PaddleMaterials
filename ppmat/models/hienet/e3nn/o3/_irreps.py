...
import paddle
from ..math import direct_sum, perm

from . import _rotation, _wigner
...
class Irrep(tuple):
    ...
    def D_from_angles(self, alpha, beta, gamma, k=None) -> paddle.Tensor:
        ...
        """
        if k is None:
            k = paddle.zeros_like(alpha)
        alpha, beta, gamma, k = paddle.broadcast_tensors([alpha, beta, gamma, k])
        return (
            _wigner.wigner_D(self.l, alpha, beta, gamma) * self.p ** k[..., None, None]
        )

    def D_from_quaternion(self, q, k=None) -> paddle.Tensor:
    ...
    def D_from_matrix(self, R) -> paddle.Tensor:
        """Matrix of the representation, see `Irrep.D_from_angles`
    ...
    def D_from_axis_angle(self, axis, angle) -> paddle.Tensor:
        """Matrix of the representation, see `Irrep.D_from_angles`
        ...
        if normalization == "component":
            ...
            with paddle.no_grad():
                for s, (mul, ir) in zip(self.slices(), self):
                    r = paddle.randn(
                        *lsize, mul, ir.dim, *rsize, dtype=dtype, device=device
                    )
                    r.div_(r.norm(2, dim=di + 1, keepdim=True))
                    x.narrow(di, s.start, mul * ir.dim).copy_(
