...
from . import _const as _const
from . import _keys as KEY
from . import util as util
import paddle
from .e3nn.o3 import Irreps
from .nn.convolution import IrrepsConvolution
from .nn.edge_embedding import (BesselBasis, ComformerEdgeEmbedding,
                                      EdgeEmbedding, EdgePreprocess,
                                      ExpNormalSmearing, PolynomialCutoff,
                                      RBFExpansion, SphericalEncoding,
                                      XPLORCutoff)
from .nn.equivariant_gate import EquivariantGate
from .nn.force_output import ForceOutputFromEdge, ForceStressOutput
from .nn.iComfLayers import (ComformerConvEdgeLayer,
                                   ComformerNodeConvLayer,
                                   TensorProductConvLayer,
                                   eComfEquivariantConvLayer)
from .nn.linear import (AtomReduce, FCN_e3nn, IrrepsDropoutLinear,
                              IrrepsLinear, get_linear)
from .nn.node_embedding import CGCNNEmbedding, OnehotEmbedding
from .nn.scale import Rescale, SpeciesWiseRescale
from .nn.self_connection import (SelfConnectionIntro,
...
def NequIP_interaction_block(
    ...
    **conv_kwargs,
    ...
    block[f"{t}_self_connection_intro"] = sc_intro(
        irreps_x, irreps_operand=irreps_node_attr, irreps_out=irreps_for_gate_in
    )
    block[f"{t}_self_interaction_1"] = IrrepsLinear(
        irreps_x, irreps_x, data_key_in=data_key_x, biases=bias_in_linear
    )
    ...
    block[f"{t}_self_interaction_2"] = IrrepsLinear(
        irreps_out_tp, irreps_for_gate_in, data_key_in=data_key_x, biases=bias_in_linear
    )
...
def build_E3_equivariant_model(config: dict, parallel=False):
    ...
    if use_iComf_embedding:
        ...
        edge_embedding = EdgeEmbedding(
            ...
            angle_module=paddle.nn.Sequential(
                RBFExpansion(r_min=-1.0, r_max=1.0, n_bins=triplet_features),
                paddle.compat.nn.Linear(triplet_features, edge_dim),
                paddle.nn.Softplus(),
            ),
        )
    if not parallel:
    ...
    layers.update(
        {
            "onehot_idx_to_onehot": CGCNNEmbedding()
            if use_CGCNN_embedding
            else OnehotEmbedding(num_classes=num_species),
    ...
    for i in range(inv_convolutional_layer):
        layers.update(
            {
                f"{i}_invariant_convolution": ComformerNodeConvLayer(
        ...
        if use_edge_conv and i == 0:
            layers.update(
                {
                    f"{i}_edge_convolution": ComformerConvEdgeLayer(
                        in_channels=edge_dim, out_channels=edge_dim, edge_dim=edge_dim
                    )
                }
            )
    ...
    rescale_module = (
        SpeciesWiseRescale if config[KEY.USE_SPECIES_WISE_SHIFT_SCALE] else Rescale
    )
    layers.update(
        {
    ...
    if not parallel:
        ...
        fof = ForceOutputFromEdge(
            data_key_energy=KEY.PRED_TOTAL_ENERGY, data_key_force=KEY.PRED_FORCE
        )
