...
@compile_mode("script")
class IrrepsConvolution(paddle.nn.Module):
    """
    ...
    def __init__(
        ...
        self.denominator = paddle.nn.Parameter(
            paddle.FloatTensor([denominator]), requires_grad=train_denominator
        )
    ...
    def forward(self, data: AtomGraphDataType) -> AtomGraphDataType:
        ...
        x = data[self.key_x]
        if self.is_parallel:
            x = paddle.cat([x, data[KEY.NODE_FEATURE_GHOST]])
        ...
        if self.is_parallel:
            x = paddle.tensor_split(x, data[KEY.NLOCAL])[0]
        data[self.key_x] = x
        return data
